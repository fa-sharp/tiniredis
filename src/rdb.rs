...
async fn parse_rdb_file(file_path: &str) -> anyhow::Result<ParsedRdb> {
    let mut file = BufReader::new(File::open(file_path).await?);
    let mut buf = BytesMut::with_capacity(1000);

    // HEADER
    buf.resize(5, 0);
    file.read_exact(&mut buf[..5]).await?; // REDIS
    file.read_exact(&mut buf[..4]).await?; // rdb version
    let rdb_version = buf.split_to(4).freeze();

    // METADATA
    let mut metadata = Vec::new();
    while file.read_u8().await? == META_HEADER {
        let n = read_length_encoded_value(&mut file, &mut buf).await?;
...
async fn read_length_encoded_value(
    ...
    let (length, n) = match (first_byte & 0b11000000) >> 6 {
        0b00 => ((first_byte & 0b00111111) as usize, 1), // length is the last 6 bits
        0b01 => {
            // length is last 6 bits and next byte
            let len_bytes = [first_byte & 0b00111111, reader.read_u8().await?];
            (u16::from_be_bytes(len_bytes) as usize, 2)
        }
        0b10 => (reader.read_u32().await? as usize, 5), // length is the next 4 bytes
        0b11 => {
            // Value is an encoded integer
            let (val, n) = match first_byte {
                0xC0 => {
                    // value is an 8-bit integer
                    (reader.read_u8().await?.to_string(), 2)
                }
                0xC1 => {
                    // value is a little-endian 16-bit integer
                    (reader.read_u16_le().await?.to_string(), 3)
                }
                0xC2 => {
                    // value is a little-endian 32-bit integer
                    (reader.read_u32_le().await?.to_string(), 5)
                }
                _ => unimplemented!(),
            };
...
mod test {
    ...
    async fn string() -> anyhow::Result<()> {
        ...
        let n = read_length_encoded_value(reader, &mut buf).await?;
        assert_eq!(n, 13);
    ...
    async fn parse() -> anyhow::Result<()> {
        let rdb = parse_rdb_file("dump.rdb").await?;
        println!("{rdb:?}");
        Ok(())
    }
}
